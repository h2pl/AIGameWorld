/** MovementManager 单元测试 / Unit tests for MovementManager */

import { describe, it, expect, vi } from "vitest";
import { MovementManager } from "../../../src/managers/MovementManager";
...
describe("MovementManager", () => {
  /** 每个用例构造独立 manager / Each test gets a fresh manager */
  function makeMgr() {
    const scene = { tweens: { getTweensOf: vi.fn(() => []) } } as any;
    return new MovementManager(scene, TILE_SIZE);
  }
  ...
  it("walkTo immediately completes if already at target", () => {
    ...
    mgr.walkTo(sprite as any, 5, 5, { onComplete: cb });
  ...
  it("prependWalkTo prepends steps to queue", () => {
    ...
    mgr.prependWalkTo(sprite as any, 2, 0);
    expect(sprite.walkQueue.length).toBeGreaterThan(0);
  ...
  it("walkRoute calls onComplete with final tile", () => {
    ...
    mgr.walkRoute(sprite as any, [{ x: 3, y: 3 }], { onComplete: cb });
