    ...
    if (spawnObjs) {
      ...
      for (const ch of st.characters) {
        ...
        if (ch.is_pc) {
          ...
          wx = (spawn?.x || 384) + pcIdx * 32; wy = spawn?.y || 320;
          pcIdx++;
        } else {
          wx = 384 + (actorIdx % 4) * 64; wy = 200 + Math.floor(actorIdx / 4) * 48;
          actorIdx++;
        }
        // 保存到 gameStore / Save to gameStore
        st.character_positions[ch.id] = { x: Math.floor(wx / 32), y: Math.floor(wy / 32) };
      }
    }
