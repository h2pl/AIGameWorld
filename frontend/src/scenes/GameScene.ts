...
import { SceneSetupHandler, type SceneSetupData } from "../managers/event_handler/SceneSetupHandler";
...
export class GameScene extends Phaser.Scene {
  ...
  destroyScene(): void { this._destroyScene(); }
  ...
  private _initHandlers(): void {
    ...
    const getSprite = (id: string) => this.pcManager?.getSprite(id) || this.actorManager?.getSprite(id);
    this.exploreHandler = new ExploreHandler(getSprite, () => this.movementManager, follow);
    this.talkHandler = new TalkHandler(getSprite, () => this.movementManager, () => this.sceneBuilt, follow);
    new NarrativeHandler((text) => this.hud.setNarrative(text));
    new InteractHandler();
  ...
  private _registerHandlers(): void {
    ...
    this.eventManager.register("pc_interact", (ev) => new InteractHandler().handle(ev));
    this.eventManager.register("dm_narrative", (ev) => new NarrativeHandler((t) => this.hud.setNarrative(t)).handle(ev));
  }
  ...
  private _recoverFromReload(): void {
    ...
    const player = this.game.registry.get("tickPlayer") as { recoverFromReload: (t: number) => Promise<void> };
    if (!player) return;
  ...
  private _buildScene(data: SceneSetupData): void {
    ...
    log.info(`build scene=${data.sceneId} map=${data.mapKey} pcs=${data.pcs.length} actors=${data.actors.length}`);
    try {
    ...
    } catch (e) { log.error(`build ERROR:`, e); }
  }
  ...
  private _buildTerrain(objects: any[]): void {
    ...
    for (const obj of objects) {
      ...
      this.terrainSprites.push(this.add.sprite(wx, wy, key)
        .setOrigin(0.5, 1).setDepth(DEPTH.CHARACTER - 1)
        .setInteractive({ useHandCursor: true })
        .on("pointerdown", () => document.dispatchEvent(new CustomEvent("object-interacted", { detail: obj }))));
    }
  }
  ...
  private _buildCharacters(pcs: any[], actors: any[]): void {
    ...
    this.cameras.main.scrollX = sx; this.cameras.main.scrollY = sy;
  }
  ...
  private _setupInput(): void {
    ...
    this.input.on("pointerdown", (p: Phaser.Input.Pointer) => {
      ...
      for (const pc of d.pcs) {
        if (Math.abs(wp.x - (pc.position_x * this.ts + this.ts / 2)) < 20 &&
      ...
      for (const actor of d.actors) {
        if (Math.abs(wp.x - (actor.position_x * this.ts + this.ts / 2)) < 20 &&
  ...
  private _destroyScene(): void {
    ...
    this.pcManager?.destroy(); this.pcManager = undefined as any;
    this.actorManager?.destroy(); this.actorManager = undefined as any;
    this.mapManager.destroy();
    this.terrainSprites.forEach((s) => { if (s.active) s.destroy(); });
    this.terrainSprites = [];
    this.hud.destroy();
    this.talkHandler?.clear();
