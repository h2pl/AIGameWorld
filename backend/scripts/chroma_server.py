...
# 项目根（backend/ 目录）——用于计算默认 persist path，不再强制 chdir 避免影响用户传的相对/绝对路径
...
def _resolve_persist_path(cli_value: str | None) -> Path:
    """解析 --persist-path 参数：绝对路径直接用；相对路径按「当前 working directory」解析；不传则默认 backend/data/chroma."""
    if cli_value:
        p = Path(cli_value).expanduser()
        if not p.is_absolute():
            # 允许用户用相对路径（相对于当前 shell cwd，而不是 backend_dir，避免和 Studio 路径混淆）
            p = Path.cwd() / p
        return p.resolve()
    # 默认：backend/data/chroma
    return (backend_dir / "data" / "chroma").resolve()
...
# ── 外层包装 app ──────────────────────────────────────────────
app = FastAPI(
    title="ChromaDB Server (AIGameWorld · 记忆库)",
...
def _build_arg_parser():
    ...
    p = argparse.ArgumentParser(
        ...
        description=(
            ...
            "支持 --persist-path 指定任意 chroma 持久化目录，可与 Studio 知识库分端口启动：\n"
            "  · 主项目记忆库（默认）: (不传 --persist-path) 本项目 backend/data/chroma，默认端口 8002\n"
            "  · Studio 知识库: --persist-path E:/Projects/AIGameWorld-Studio/data/chroma --port 8001"
        ),
    ...
    p.add_argument(
        "--persist-path",
        default=None,
        help="ChromaDB 持久化目录（绝对/相对路径均可；相对路径按当前 shell cwd 解析；不传默认 backend/data/chroma）",
    )
    p.add_argument(
        "--log-level",
