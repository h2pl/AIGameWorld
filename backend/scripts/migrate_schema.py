...
async def m():
    ...
    await db.execute(
        "CREATE TABLE IF NOT EXISTS dm_records (id INTEGER PRIMARY KEY AUTOINCREMENT, world_id TEXT NOT NULL, tick INTEGER NOT NULL, plot_brief TEXT NOT NULL DEFAULT '', hints_json TEXT NOT NULL DEFAULT '[]', dm_narrative TEXT NOT NULL DEFAULT '', created_at TEXT NOT NULL DEFAULT (datetime('now')), updated_at TEXT NOT NULL DEFAULT (datetime('now')), UNIQUE(world_id, tick))"
    )
    await db.execute(
        "CREATE INDEX IF NOT EXISTS idx_dm_records_world_tick ON dm_records(world_id, tick)"
    )
    await db.execute(
        "CREATE TABLE IF NOT EXISTS story_summaries (id INTEGER PRIMARY KEY AUTOINCREMENT, world_id TEXT NOT NULL, tick_start INTEGER NOT NULL, tick_end INTEGER NOT NULL, summary TEXT NOT NULL, created_at TEXT NOT NULL DEFAULT (datetime('now')), UNIQUE(world_id, tick_start))"
    )
