...
只做 Graph 编排 / Graph orchestration only:
  character_service.observe_scene → character_service.decide → character_service.act → END
"""
...
def build_character_subgraph() -> StateGraph:
    ...
    graph.add_node("character_service.observe_scene", character_service.observe_scene)
    ...
    graph.set_entry_point("character_service.observe_scene")
    graph.add_edge("character_service.observe_scene", "character_service.decide")
