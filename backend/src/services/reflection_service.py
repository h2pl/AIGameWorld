...
async def reflect(state: ReflectionSubState, config: RunnableConfig = None) -> dict:
    ...
    for actor in await char_repo.load_actors():
        if _needs_reflection(memory_repo, actor.id, _ACTOR_THRESHOLD):
            insight = await _reflect_one(reflection_engine, actor.id, actor.name, "actor",
...
async def _reflect_one(engine, char_id: str, name: str, char_type: str,
                       arc_stage: str, arc_desc: str, memory_repo, tick: int,
                       config) -> dict | None:
    ...
    result = await engine.reflect(ReflectionRequest(
