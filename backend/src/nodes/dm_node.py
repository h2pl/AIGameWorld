"""DM Node: State <-> Service glue / DM 节点：State ↔ Service 胶水。

职责 / Responsibility:
  - 读取 graph State (plain dict)
  - 构造 Service 输入 (DMSubState)
  - 调用 Service 纯函数 (dm_create / dm_narrate)
  - 将结果映射回 graph State key
"""
from typing import Any

from ..engine.dm.dm import DMSubState, dm_create, dm_narrate
...
def dm_create_node(state: dict[str, Any]) -> dict[str, Any]:
...
def dm_narrate_node(state: dict[str, Any]) -> dict[str, Any]:
