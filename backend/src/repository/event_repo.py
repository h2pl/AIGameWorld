...
class TickEventRepo:
    ...
    async def insert_tick_events(
        self, tick_message_id: str, tick: int, tick_events: list[dict | TickEvent]
    ) -> None:
        """批量写入事件."""
        for ev in tick_events:
            ...
            await self._db.execute(
                "INSERT INTO tick_events (tick_message_id, tick, type, payload) VALUES (?, ?, ?, ?)",
    ...
    async def insert_events(
        ...
        """兼容旧接口 / Legacy alias."""
        await self.insert_tick_events(tick_message_id, tick, events)
    ...
    async def load_by_tick_range(self, world_id: str, tick_start: int, tick_end: int) -> list[dict]:
        ...
        rows = await self._db.fetch_all(
            ...
            "JOIN tick_messages m ON e.tick_message_id = m.id AND e.tick = m.tick "
