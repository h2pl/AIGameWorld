...
def trace_node(name: str = ""):
    """节点追踪装饰器——结构化日志 / Node tracing decorator: structured logging.

    - 始终记录结构化日志（log_node）
    - Langfuse/LangSmith 的 trace 由 LangGraph CallbackHandler 自动捕获
      （CallbackHandler 内部用 runId/parentRunId 维护父子关系，不依赖 OTel context，
       所以不要在此处用 start_as_current_observation 创建独立 span——会导致 trace 分散）
    """
