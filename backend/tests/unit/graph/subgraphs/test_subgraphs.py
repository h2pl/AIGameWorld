...
class TestEngineSubgraph:
    ...
    async def test_runs_without_crash(self):
        state: EngineSubState = {
            "participants": [], "round": 1, "speaker": "", "target": "",
    ...
    async def test_accumulates_results(self):
        ...
        state: EngineSubState = {
            "participants": ["hero", "goblin"], "round": 1,
            "speaker": "pc1", "target": "npc1", "intent": "persuade",
            "character_id": "pc1", "action_type": "search",
            "quests": [{"id": "q1"}], "event_log": [], "engine_results": [],
            "combat_result": None,
        }
        result = await engine_subgraph.ainvoke(state)
