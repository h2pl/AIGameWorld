...
class TestDMCreate:
    ...
    async def test_creates_situation_with_llm(self):
        ...
        result = await dm_engine.dm_create(DMCreateRequest(tick=1, plot_brief=""), {"configurable": {"llm": llm}})
    ...
    async def test_fallback_on_llm_failure(self):
        ...
        result = await dm_engine.dm_create(DMCreateRequest(tick=0, plot_brief=""), {"configurable": {"llm": llm}})
...
class TestDMNarrate:
    ...
    async def test_narrates_with_llm(self):
        ...
        llm = AsyncMock()
        llm.call_structured = AsyncMock(return_value=DMNarrativeSchema(narrative="LLM narrative text"))
        req = DMNarrateRequest(tick=0, plot_brief="Story", dm_instructions=[],
                               scene_direction={}, character_actions=[])
        result = await dm_engine.dm_narrate(req, {"configurable": {"llm": llm}})
...
class TestDMGuardrails:
    def test_validate_dm_output_fixes_invalid_mood(self):
        ...
        result = DMOutput(plot_brief="x", scene_direction=SceneDirectionOutput(mood="invalid_mood"), instructions=["a"])
    ...
    def test_validate_dm_output_keeps_valid_mood(self):
        from src.engine.dm.dm import _validate_dm_output
        result = DMOutput(plot_brief="x", scene_direction=SceneDirectionOutput(mood="tense"), instructions=["a"])
        fixed = _validate_dm_output(result)
    ...
    def test_validate_dm_output_truncates_excess_instructions(self):
        from src.engine.dm.dm import _validate_dm_output
        result = DMOutput(plot_brief="x", scene_direction=SceneDirectionOutput(), instructions=["a"] * 6)
        fixed = _validate_dm_output(result)
